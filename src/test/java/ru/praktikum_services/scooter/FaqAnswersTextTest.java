...
public class FaqAnswersTextTest extends BaseTest{
    ...
    public FaqAnswersTextTest(int indexOfQuestion, String expectedAnswers) {
        this.expectedAnswers = expectedAnswers;
        this.indexOfQuestion = indexOfQuestion;
    }
    // Тестовые данные, создаем лист с листами для корректной параметризации
    @Parameterized.Parameters()
    public static Object[][] getQuestionAndAnswerWithOrder() {
        return new Object[][]{
            {
    ...
    public void checkMatchingQuestionsAndAnswers() {
        String actualAnswer = new HeaderFaq(driver)
                    .clickToQuestionPanel(indexOfQuestion)
                    .getNotHiddenAnswer();
        assertEquals("Действительный ответ не совпадает с ожидаемым", expectedAnswers, actualAnswer);
    }
}